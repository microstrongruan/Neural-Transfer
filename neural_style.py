...
#            metavar='OUTPUT', required=True)
    parser.add_argument('--content',
            dest='content', help='content image',
            metavar='CONTENT', default = 'examples/1-content.jpg')
    parser.add_argument('--styles',
            dest='styles',
            nargs='+', help='one or more style images',
            metavar='STYLE', default = ['examples/1-style.jpg']) # 坑，输入进来是当成一个列表
    parser.add_argument('--output',
            dest='output', help='output path',
            metavar='OUTPUT', default = 'examples/1-test_output.jpg')
...
def main():
    ...
    for i in range(len(style_images)):  # 这里把style_images进行了一个比例的缩放 0.625
    ...
    if style_blend_weights is None:
        # default is equal weights 默认情况是同等权重
